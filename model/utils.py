...
def sample(logits: torch.Tensor, temperature: float = 0.0):
    ...
    return torch.multinomial(probs, num_samples=1).view(bsz, seq_len), probs

def unmask(block_ids: torch.Tensor, probs: torch.Tensor, threshold: torch.float = 0.8):
    prob_max = torch.gather(probs, dim=-1, index=block_ids.squeeze(0).unsqueeze(-1)).squeeze(-1)
    #print("prob_max:", prob_max)
    return (prob_max >= threshold).cumprod(dim=-1).sum()
